...
### 2.7 ระบบ KDS จอครัว (Kitchen Display System) (`apps/admin/app/kds`)
- หน้าจอ KDS ที่ `https://admin.bcaicloud.com/kds` (มีปุ่มสลับหน้าจอใน Topbar ของ Dashboard)
- การ์ดออเดอร์พร้อมนาฬิกาจับเวลาถอยหลัง/เวลารออาหาร (เขียว <10น., ส้ม 10-15น., แดงกะพริบ >15น.)
- แตะเพื่อขีดฆ่าจานที่ทำเสร็จแล้วทีละรายการ (Item check-off)
- ปุ่มเปลี่ยนสถานะ: กำลังปรุง (Cooking) → พร้อมเสิร์ฟ (Ready) → เสิร์ฟแล้ว (Served)
- ตัวกรองสเตชันครัว: ไก่ทอดเกาหลี, อาหารเกาหลี, ของทานเล่น, บิงซู & เครื่องดื่ม
- เสียงกริ่งสังเคราะห์ (Web Audio API) แจ้งเตือนเมื่อมีออเดอร์ใหม่เข้าครัวแบบเรียลไทม์ (Socket.IO)
- Backend API: `PATCH /api/menu/orders/:id/status` และ Socket.IO event `order:update`
