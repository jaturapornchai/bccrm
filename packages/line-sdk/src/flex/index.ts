...
const bubble = (headerText: string, headerColor: string, bodyContents: unknown[], footerUrl: string, footerLabel: string) => ({
  type: "bubble",
  header: {
    type: "box",
    layout: "vertical",
...
export function ticketCardFlex(data: TicketCardData): FlexMessage {
  return {
    type: "flex",
    altText: `บัตรคิว ${data.ticketNumber} — เหลืออีก ${data.aheadCount} คิว`,
    contents: bubble(
      `🎫 บัตรคิว ${data.ticketNumber}`,
      "#06C755",
      [
...
/** แจ้งเตือนใกล้ถึงคิว */
export function almostThereFlex(data: TicketCardData): FlexMessage {
  return {
    type: "flex",
...
/** เรียกคิว — ถึงคิวแล้ว */
export function calledFlex(ticketNumber: string, counterName: string, ticketUrl: string): FlexMessage {
  return {
    type: "flex",
...
/** ยกเลิกคิวสำเร็จ */
export function cancelledFlex(ticketNumber: string, rebookUrl: string): FlexMessage {
  return {
    type: "flex",
